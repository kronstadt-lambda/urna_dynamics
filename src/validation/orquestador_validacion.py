...
class OrquestadorForense:
            ...
            if ruta.exists():
                with open(ruta, "r", encoding="utf-8") as f:
                    ...
                    for v in datos:
                        ...
                        # CAMBIO: Usamos el nombre completo como identificador único
                        nombre = v['name']
    ...
    def _generar_metricas_macro(self, df: pd.DataFrame, top_k: int = 10):
        ...
        print(f" [GLOBAL] G-Stat Estabilizado         : {reporte_global['G_Stat_Macro_Estabilizado']}")
    ...
    def ejecutar_analisis(self):
        ...
        # Filtro de integridad física puro (sin corrección de etiquetas)
        ...
        for obj in objetivos:
            ...
            # Límite inferior de seguridad: Al menos 2 personas o 1/3 del partido para evitar ruido
            limite_inferior = max(2, int(total_miembros * 0.33))

            print(f"\n[*] Evaluando: {partido} -> {opcion} (Total: {total_miembros} miembros)")

            # Iteramos bajando la cantidad de personas (excluyendo de 0 hasta llegar al límite inferior)
            for k_seleccionados in range(total_miembros, limite_inferior - 1, -1):
                ...
                for combo in tqdm(combinaciones, desc=f"      Excluyendo {num_excluidos} ({k_seleccionados} activos)", leave=False):
                    df_sim_subset = df_partido_sim[df_partido_sim['name'].isin(combo)]
